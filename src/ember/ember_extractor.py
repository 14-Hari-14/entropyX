...
class SectionInfo(FeatureType):
    ...
    def raw_features(self, bytez, lief_binary):
        ...
        entry_section = ""
        try:
            # Try to find the section containing the entry point
            if int(LIEF_MAJOR) > 0 or (int(LIEF_MAJOR) == 0 and int(LIEF_MINOR) >= 12):
                ...
                if section is None:
                    raise Exception("Section not found") # Replaced lief.not_found
                entry_section = section.name
            else: # lief < 0.12
                entry_section = lief_binary.section_from_offset(lief_binary.entrypoint).name
        except Exception: # Catch generic errors instead of lief.not_found
            # Fallback: bad entry point, let's find the first executable section
            entry_section = ""
            for s in lief_binary.sections:
                # Safe check for MEM_EXECUTE that works on ALL LIEF versions
                # We convert the characteristics list to a string to avoid Enum attribute errors
                if "MEM_EXECUTE" in str(s.characteristics_lists):
...
class PEFeatureExtractor(object):
    ...
    def raw_features(self, bytez):
        # FIX: Replaced specific LIEF exceptions with a generic catch for compatibility
        try:
            lief_binary = lief.PE.parse(list(bytez))
        except (RuntimeError, Exception) as e:
            # LIEF throws RuntimeError for C++ parsing issues, and other exceptions for Python issues
            print("lief parsing error: ", str(e))
            lief_binary = None

        # Calculate SHA256 (Standard logic)
        features = {"sha256": hashlib.sha256(bytez).hexdigest()}
        
        # Extract all feature groups (Standard logic)
